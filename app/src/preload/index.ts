...
contextBridge.exposeInMainWorld('amber', {
  ...
  openPiPane: (session: string) => ipcRenderer.send('open-pi-pane', session),
  ...
  closePiPane: (session: string) => ipcRenderer.send('close-pi-pane', session),
...
ipcRenderer.on('pane-port', (e, meta: { session: string; mode?: 'terminal' | 'pi' }) => {
  const port = e.ports[0]
  if (!port) return
  window.postMessage({ amberPanePort: true, session: meta.session, mode: meta.mode ?? 'terminal' }, '*', [port])
})
