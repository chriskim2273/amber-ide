...
workspace, and running Codex conversation survives app crashes AND
...
2. **Every app pane is its own daemon session** — one pty running exactly one
   process (`$SHELL`, or `amber run <name>` which supervises `Codex`). Tabs and
...
6. **Persistence is the daemon's job, not the app's.** Crash survival: the
   ...
   continuum.
7. **Codex is supervised, resumed precisely.** A `Codex` session's pty runs
   `amber run <name>`, which loops `Codex --dangerously-skip-permissions
   ...
   every fire (ids rotate on resume/clear/compaction). Codex is resolved via
...
9. **Pi graphical-view exception:** a supervised `kind:"pi"` pane may replace
   its renderer-side terminal attachment with Amber's optional semantic chat
   view. The existing daemon session, PTY, supervisor, and single Pi TUI process
   remain authoritative and alive; the view uses Amber's public-API Pi extension
   sideband over the authenticated Amber transport. It must never scrape TUI
   bytes, start a second Pi process, expose a Pi-owned network listener, or let
   two processes write one Pi session file. Terminal remains the default and
   must remain available without behavior changes.
...
connection manager; generic or non-Pi AI chat UI (the narrow Pi graphical-view
exception is core rule 9); themes/settings beyond minimal.
...
- `crates/amber/` — `pty` (owned child + fan-out), `manager` (save/restore),
  `daemon` (socket server), `attach` (raw-mode client), `Codex` + `supervisor`
...
- [x] Slice 2 — Codex supervision (`amber run`/`hook`, resume/continue,
...
- [x] App Slice 6 — workspace switcher; Codex panes (`kind=Codex`); reconnect
  ...
  Fixes shipped: Codex fresh-start for new sessions; deferred split placement;
...
- [x] Gap-fix pass (2026-07-13) — closed the review gaps: spec §5 raw-attach
  semantics (`raw_client` Attach flag, terminal reset, Codex backlog skip +
  ...
  the daemon's unsupported error); `Codex.rs` warns-and-skips wrong-shaped
...
- [x] UX pass (2026-07-14) — draggable panels: a grip in each pane's control
  ...
  terminals. Codex user-quit now drops to a shell: a clean exit, exit code 130
  (Codex's raw-mode ^C path), OR death by SIGINT all classify as a user quit in
  ...
  bundle builds. Still open: the live drag gesture + Codex ^C→shell need
...
- [x] Attach session indicator + nesting refusal (2026-07-14) — `amber attach`
  ...
  sessions (a Codex session is a full-screen TUI already on the alt screen the
...
- [x] UI/UX pass (2026-07-17) — 10 reviewed tasks (subagent-driven; plan:
  ...
  suppressed; superseded for Codex panes by the freeze grace below, which
  suspends the child after 30 s). Daemon: `ReportRunState`
  (supervisor fire-and-forget) + `SessionInfo.run_state` → Codex panes show
  Codex/retrying/shell-fallback dots; rate-limited (500 ms) `Activity`
...
- [x] Workspace save/load (2026-07-17) — manual save/restore of workspaces to
  ...
  prop churn). Codex panes load as fresh Codex (no resume; history under
...
- [x] Split kind picker + cross-tab/ws pane move (2026-07-19) — **split**: the
  pane header's ⬌/⬍ now open a shell/Codex/browser picker (reuses the ctx-menu
  ...
  **Move**: drag a pane's ⠿ grip onto a tab header, workspace pill, `+ Tab`, or
  ...
  `StateStore::rename_session` (moves sessions/Codex/settings/scrollback
  ...
  renamed IN PLACE keeping its child + scrollback, Codex respawned via
  ...
  thread inherits another's write fd to its fake-Codex script → `ETXTBSY` on
  ...
  private daemon: picker splits each kind, Codex pane moved tab→ws (name
...
- [x] `amber web` — phone browser access (2026-07-19) — a mobile web UI for live
  sessions: list → tap → full-screen xterm → type (drives Codex too). Spec:
  ...
  **client** (rule #1 intact; the daemon still never binds a network interface),
  ...
  resize would reflow live work and corrupt a Codex TUI — the front end has no
...
- [x] Wedged-client freeze fix (2026-07-20) — **the daemon froze while idle**,
  ...
  recovered it. Root cause (the ticket AGENTS.md had left open since the
...
- [x] Dangling global-hook GC (2026-07-19) — `ensure_global_claude_hook` dedupes
  ...
  `SessionStart` entry in `~/.Codex/settings.json`. Dev builds run out of git
  worktrees therefore accumulated entries, and deleting a worktree left Codex
...
- [x] Freeze grace — suspend/resume Codex (2026-07-19, commit 49c3d8c) —
  freezing a **Codex** pane frees its RAM: the app sends `Suspend`
  ...
  supervisor, which kills Codex, reports `run_state:"suspended"` (amber pane
  ...
  hook-recorded id and relaunches `Codex --resume <id>` — the SAME
  conversation, not a fresh one. The kill is NOT counted against the crash
  budget. Non-Codex/unknown sessions reply `Error`; shell panes freeze
  ...
  app 389 tests + typecheck. Live Codex round-trip (freeze → real Codex
...
- [x] Adopt + bare `amber` (2026-07-23) — a CLI session and a pane are now the
  ...
  Codex adoption rides the already-proven cross-tab rename path (not re-tested).
...
- [x] Grok session kind (2026-07-26) — a second supervised agent: `kind:"grok"`
  ...
  than Codex support because grok's id does NOT rotate and amber can ASSIGN it:
  ...
  GC. The id is recorded in the SAME `Codex/<name>.json` (rename/kill/adopt
  ...
  Codex's one: measured, a just-killed pane can 404 ("not found locally,
  ...
  races it — observed live launching **Codex** for a grok pane. Kind-gated
  ...
  `Codex*` for both agents (they name the phase, not the binary). Scope cuts:
  no global grok hook, no hand-started-grok detection (`resume_as_claude` is
  still Codex-only), and the cleanup dialog's conversation labels read Codex
...
- [x] Compat-mode false positive (2026-07-26) — **the "app gets laggy after a
  ...
  **Trigger identified (not amber's bug):** `systemd-oomd`. At 03:53:56 it killed
  ...
  cascade then took Codex Desktop (03:54:43, `Killed`), Firefox's whole scope
...
- [x] Pane display env (2026-07-29) — **image paste into a Codex pane did
  nothing**. Codex reads the clipboard ITSELF: on Linux it shells out to
  ...
  Codex's own `2>/dev/null` — a silent no-op, no error anywhere. **Why it
  ...
  **A running daemon must be restarted to pick this up** — a live pane's env is
  ...
  plain Ctrl-V is the gesture that works (it forwards `^V` to Codex, which does
  its own clipboard read). Not fixed, upstream: Codex's probe accepts
...
- [x] Memory audit (2026-07-31) — full-repo leak + allocation audit across the
  ...
  sessions, cgroup `memory.current` 15.65 GB — of which ~7 GB is Codex children,
...
- [x] Codex session kind (2026-08-10) — a third supervised agent: `kind:"codex"`
  ...
  `docs/superpowers/specs/2026-08-10-codex-session-kind-design.md`. Codex-shaped
  ...
  `Codex/<name>.json` store. Fresh: `codex --dangerously-bypass-approvals-and-sandbox
  --dangerously-bypass-hook-trust`; resume: `codex resume <id> …`. Never
  `resume --last`. Kind-gated sites use `SessionKind::is_agent()` /
  `isAgentKind()` (codex included). Run-state vocabulary stays `Codex*`.
  ...
  **A running daemon must be restarted to accept `kind:"codex"`.** Fake-Codex
  ...
  and the interactive picker. `$Codex-handoff <id>` is installed as a
  user-level Codex skill; fake-Codex tests cover its safe, non-persistent
...
- [x] Optional Pi graphical pane (2026-09-02) — approved narrow exception to
  the generic AI-chat exclusion. A persisted per-session `Terminal | Chat`
  presentation toggle keeps the existing supervised Pi process and PTY alive,
  while Amber's owned Pi extension emits bounded semantic snapshots/events over
  the local daemon socket using only public Pi APIs. New controls are explicitly
  capability-gated (`WatchPiEvents`), daemon/web/client queues remain bounded,
  desktop uses dedicated MessagePorts, and authenticated `amber web` exposes a
  semantic-only mobile WebSocket mode that never attaches or resizes the PTY.
  The React view safely renders text/Markdown structure, thinking and tool
  details, streaming replacement, model/thinking/context status, prompt
  delivery, Stop, reconnect snapshots, and a terminal handoff for extension UI
  prompts. Terminal remains the absent/default sidecar value. Design and plan:
  `docs/superpowers/specs/2026-09-02-pi-gui-pane-design.md` and
  `docs/superpowers/plans/2026-09-02-pi-gui-pane.md`. Release gates verified
  2026-09-03: workspace Rust tests 823 passed / 0 failed / 1 ignored, clippy
  warnings-as-errors passed, and the app suite 760 passed / 1 skipped across
  61 passed / 1 skipped files with typecheck, desktop build, and web build
  green. The generated hook was typechecked against Pi API 0.84.4; private
  `/tmp/amber-pi-gui-live` semantic smoke proved registration, snapshot,
  safe thinking command, reconnect snapshot, no binary PTY delivery in GUI
  mode, and terminal fallback bytes, with one supervised Pi child. No
  production daemon/state was touched. Remaining manual checks are real-device
...
- A TUI (Codex) respawned **detached at boot** must get a real pty size (never
...
- **TDD**: write the failing test first, watch it fail, then minimal code.
  Unit-test the pure parts (ring, proto, state, Codex argv/hook); integration
