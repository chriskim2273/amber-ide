...
After the uniform-input generation fix at code HEAD `c5b87c8`, fresh gates under `/tmp/amber-tab-browser-host-validation` recorded: `npm test` **1001 passed / 1 intentional real-daemon skip** (81 files); strict `npm run typecheck`; `npm run build`; `npm run build:web`; Rust `cargo test --workspace --all-targets` **821 passed / 1 intentional delegated-cgroup ignore** (36 suites); and warnings-as-errors workspace Clippy. The focused Rust socket test `focus_refreshes_use_and_resumes_only_memory_suspension` passed **5/5** after `b832761`; the fixture-only synchronization diagnosis remains confined to that focus harness and no production Rust behavior was changed. Repository-wide rustfmt remains the documented pre-existing formatter drift and lint remains unavailable because of the ESLint-9 flat-config gap. The Linux package chain `npm run dist` produced AppImage SHA-256 `9cde8e497113320c32faf00c002850b268a8f678486821d8f66fe7e7e1b6fa2e`; unpacked `amber` and `amber-router` are static-pie. The packaged Pi extension v7 verifier reports `installedBytes:28111`, `compiled:true`, `loaded:true`, `labeledResults:true`, `fatalTokenAndFrameChecks:true`, and all 27 tools. Package approval evidence is exactly `logs/broker-approval-packaged-final.log` under that private root; its renderer input substrate is Xvfb/XTest only, not physical-hardware evidence.
