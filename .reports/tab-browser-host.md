...
A fresh `git archive HEAD` mirror was created at `source-accept` (archive SHA-256 `4c5cf068b460b9992144c4b5aeeb223cac2210ddddb21a4e1b52d0991ab241d5`). `npm ci` and the real `npm run dist` chain passed with all mutable caches/staging under the private root. The final AppImage is:
...
The pinned 0.81.0 package at `/home/poyto/.pi/agent/npm/node_modules/@earendil-works/pi-coding-agent` passed the exact `npm run test:pi-browser-extension` verifier against the packaged Amber: `installedBytes:25325`, `compiled:true`, `loaded:true`, `labeledResults:true`, `fatalTokenAndFrameChecks:true`, and all 27 browser tools registered. Because the mandated private root makes the verifier's long Unix socket path exceed AF_UNIX's 108-byte limit, `TMPDIR` pointed to a short `/tmp/ambertab-…` symlink whose resolved target remained inside the unique private root; the alias was removed immediately. Evidence: `ledger/pi-081-short-verifier-tail.txt`, `ledger/pi-package-versions.txt`, `ledger/pi-current-verifier-tail.txt`.
...
After the source-aware input fix, the current `/tmp` app gates passed: `npm test` **1002 passed / 1 intentional real-daemon skip** (82 files), strict `npm run typecheck`, `npm run build`, `npm run build:web`, and the focused input/automation/Host set (**62 passed**). The Rust all-target workspace suite passed with the private validation environment (**821 passed, 1 intentional delegated-cgroup ignore**), and workspace warnings-as-errors Clippy passed. The focused Rust socket suite passed **28/28 three times** and the focused `focus_refreshes_use_and_resumes_only_memory_suspension` test passed **5/5** with the private HOME that reproduced the prior failure. The harness race was diagnosed: Bash/dash can leave a signal trap pending while an interactive read is idle; after daemon Focus clears the suspension origin, a fixture-only empty Data line wakes that read, and the assertion still requires the Focus-triggered `SOCKET_RESUME_SIGNAL`. No production Rust behavior changed. The historical failed logs remain `logs/full-rust-tests-final.log`, `logs/full-rust-tests-final-retry.log`, and `logs/rust-flaky-targeted.log`; the successful follow-up was run in the isolated worktree/private roots. Repository-wide rustfmt remains the documented pre-existing formatter drift and lint remains unavailable because of the ESLint-9 flat-config gap.

The source-aware fix (`a8820fa fix(app): account for synthetic browser input events`) replaces the broad `runtime.automationInput` flag with bounded ordered keyboard/pointer signatures. CDP dispatches register one-shot expectations before each Input-domain command; Electron consumes only an exact callback, while physical keyboard, pointer, and composition callbacks advance generation. Successful late callbacks are bounded by expiry and a new interaction scope clears prior expectations without letting concurrent observations disturb an active dispatch; cancellation, invalidation, and dispatch errors clear immediately. Host postflight now reports the final observed generation and returns no-rollback errors when a dispatched interaction is invalidated. Tests cover keyboard, pointer, composition, drag sequences, late callbacks, cancellation, concurrent observation, physical overlap, approval-generation recheck, and stale-token cleanup. The actual packaged approval log reference is `logs/broker-approval-packaged-final.log`; all report evidence paths resolve against the unique validation root (38/38 checked).

Defect-fix commits made during this validation, each conventional and without `Co-Authored-By`, are: `58b205d fix(app): fall back when WebGL is unavailable`; `2889f06 fix(app): prefer DOM identities for browser targets`; `b5189c1 fix(app): use valid accessibility backend parameters`; `ad9d334 fix(app): isolate automation input generations`; `ace60fa fix(app): preserve browser approval surfaces`; `40496a8 fix(app): wait for daemon sessions before revoking Pi`; `642caa0 fix(app): honor isolated Electron data paths`; `a8820fa fix(app): account for synthetic browser input events`; and `af83f70 fix(app): preserve input ledger during observations`. The fixture synchronization and report/ledger corrections are committed in `b832761`, `b1b5d27`, `29d9e6e`, and `9e84a3c`. `mergeReady` remains false until the deployed-reader channel, independent resident re-review, physical/manual Linux gate, and macOS gate are closed.
