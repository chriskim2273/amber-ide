...
> **User-authorized local integration:** [Merge and staged deployment report](tab-browser-host-merge.md) records reconciliation with current main, 1074 app / 876 Rust tests, the combined AppImage and deployment sequence. Local merge was explicitly approved while production enablement remains externally gated.
...
6. Expand process-lifecycle and broker end-to-end tests around daemon reconnect races and orphan durable records after sidecar CAS conflict. **Closed by the 2026-09-04 private process-level E2E:** real Unix-socket daemon watcher disconnect/reconnect epochs and spawned sidecar CAS-conflict/process-restart recovery all pass; stale authority, duplicate browser records, and unrecoverable orphans are absent.
...
The production code under test is commit `c5b87c8`. The prior documentation/report HEAD was `e856624`; the current follow-up adds only verifier/tests and metadata, with no unrecorded production behavior change. Fresh isolated gates at this follow-up pass report `npm test` **1003 passed / 1 intentional real-daemon skip** (81 files); strict `npm run typecheck`; `npm run build`; `npm run build:web`; Rust `cargo test --workspace --all-targets` **821 passed / 1 intentional delegated-cgroup ignore** (36 suites); and warnings-as-errors workspace Clippy. The focused Rust socket test `focus_refreshes_use_and_resumes_only_memory_suspension` passed **5/5** after `b832761`; the fixture-only synchronization remains confined to that focus harness. A c5-built private Amber binary (`f99567881863ba4775b1f828606557cc30530a6df21949b6b82d1070b7c119d1`) passed the Pi v7 production-loader verifier: `installedBytes:28111`, `compiled:true`, `loaded:true`, `labeledResults:true`, `fatalTokenAndFrameChecks:true`, and all 27 tools. Evidence is `logs/pi-followup-verifier.log`, `ledger/pi-followup-verifier-rc.txt`, and `ledger/pi-followup-amber-sha256.txt` under the private root. The older packaged AppImage evidence in Gate B was built from a pre-c5 archive and remains platform/substrate evidence only; it is not attributed to c5 or Pi v7. Repository-wide rustfmt remains the documented pre-existing formatter drift and lint remains unavailable because of the ESLint-9 flat-config gap.

Electron cannot provide a trustworthy physical-versus-CDP source marker, so every `before-input-event` and `before-mouse-event` callback advances the page generation. There is no signature/token ledger: identical physical and CDP-shaped callbacks, composition, drag events, out-of-order callbacks, and late callbacks all increment independently. Host interaction dispatch validates the prepared generation immediately before the first irreversible Input-domain command, increments before dispatch, then permits later input/navigation increments and returns the final observed page incarnation/generation plus an `interleaved` indicator. The adapter reports a bounded typed failure carrying `dispatched`; Host and broker normalize any partial dispatch to `ACTION_FAILED_NO_ROLLBACK`, `retryable:false`, current context, and a fresh-snapshot instruction. The generated Pi extension preserves only this structured, secret-safe result and tells the caller to snapshot fresh. The Pi verifier now parses that result and asserts exact `code`, `retryable`, `message`, `pageIncarnation`, `generation`, `snapshotHint`, `dispatched`, and `nextStep` fields with no secret leakage. Service coverage rejects a queued stale-generation interaction before adapter dispatch, while Host coverage exercises drag, type, and fill with callbacks during every dispatch and confirms monotonic final generations without false stale errors. New validation metadata records `CODE_HEAD`/`CODE_TESTED_COMMIT` as `c5b87c8` and `REPORT_HEAD`/`REPORT_BASE_COMMIT` as `e856624`, alongside the follow-up evidence files above. The normalized environment/head record is `validation.env`; the code/report distinction is `ledger/code-vs-report-head.txt`, and the JSON/test summary is `ledger/remaining-json-validated.json` plus `ledger/followup-tests-summary.txt`; the normalized gate rollup is `ledger/gate-summary.json`. Report evidence paths resolve against the unique validation root.

Defect-fix commits made during this validation, each conventional and without `Co-Authored-By`, are: `58b205d fix(app): fall back when WebGL is unavailable`; `2889f06 fix(app): prefer DOM identities for browser targets`; `b5189c1 fix(app): use valid accessibility backend parameters`; `ad9d334 fix(app): isolate automation input generations`; `ace60fa fix(app): preserve browser approval surfaces`; `40496a8 fix(app): wait for daemon sessions before revoking Pi`; `642caa0 fix(app): honor isolated Electron data paths`; `a8820fa fix(app): account for synthetic browser input events`; `af83f70 fix(app): preserve input ledger during observations`; `6e8ecbb fix(app): use uniform browser input generations`; and `c5b87c8 fix(app): expose typed browser action errors`. The follow-up P2 tests/verifier are in `3ef779f` and `74f3c7f`, with type-fixture corrections in `8dc31dd` and `63ad66f`, and the fixture synchronization plus metadata/report corrections include `992b61c`; all follow-up changes are conventional commits. `mergeReady` remains false until the deployed-reader channel, independent resident re-review, physical/manual Linux gate, and macOS gate are closed.

## Resident review findings follow-up (2026-09-04; implementation complete, independent review pending)

This follow-up closes the requested resident P1/P2 findings without changing the daemon protocol, daemon/profile configuration, or global Pi installation:

- Forced explicit Quit now awaits the same bounded tracked-tunnel cleanup as normal Quit before calling `app.exit(1)`. Cleanup clears ownership before awaiting, waits for graceful termination, escalates to bounded `SIGKILL`, removes each owned runtime directory, and is idempotent when normal/forced paths race. The compat-restart `app.exit(0)` path uses the same awaited cleanup. A spawned-child regression proves process termination and private runtime-state removal.
- Local resident activation uses one `createSingleFlight` promise. The existing-window check runs inside the flight, all concurrent callers receive one completion, the flight resets in `finally` after rejection, and a quit fence disposes a window that finishes opening after drain begins. A closed-window native-page reparent guard prevents a destroyed prior BrowserWindow from producing an unhandled rejection during reopen.
- `TabBrowserService.windowHidden()` now derives the union of live browser IDs, activation-controller IDs, and queue-owner IDs, then runs one shared surface-revocation helper before hiding live views. A capacity-waiting thaw is cancelled and cannot create a page after capacity is released; an explicit later `show` still requeues normally. Optional browser-ID scoping leaves unrelated surfaces untouched.
- `app/test/process-fixtures/residentProcessE2E.test.ts` runs real child-process/socket coverage: a spawned Unix-socket peer drives `Connection`/`BrowserDaemonWatcher` disconnect→reconnect epochs, while spawned sidecar helpers race a real Node CAS writer and stage a real transaction before process exit; a fresh process recovers it. Assertions cover stale authority, one browser record, CAS winner preservation, pending-journal clearing, and orphan removal.

### Current gates and packaged artifact

The current code HEAD is `2fe2fccfeacab9e134c51ee90dc68d47df30ad22`, with implementation commits `a19b97f`, `1d97984`, `4c50e55`, and `2fe2fcc`. Fresh `npm run dist` built this exact AppImage in a disposable `/tmp` staging root:

`/tmp/amber-tab-browser-validation/resident-followup-20260904T225800-3480221/amber-ide-0.0.2.AppImage`

SHA-256: `fd84fabcb83f5d2321f2053cb1c7bffad9fd5c1c3f92b6576b981168e4cc9cb7`.
The unpacked resources contain executable static-musl `amber` (`a2b48e3d4bef35c4d304089af4a59343e745c628ab65c845d04612dac38833bb`) and `amber-router` (`e36374e7b0f7d53cf9c4a7dc3d7fed33d3fa6b67c9b7a3a7551d02ec92f20cdf`), with no unintended browser executable. The exact pinned Pi 0.81 production-loader verifier passed all 27 tools, fatal token/frame checks, and partial-dispatch checks (`pi-verifier.log`).

Full isolated gates: app `1012 passed / 1 intentional real-daemon skip`, strict typecheck, Electron build, hosted-web build, Rust all-target tests passed on the clean retry with **821 passed, 0 failed, 1 ignored** (the library target contributed 477 passed; the remaining integration/target suites are included in the 821 total; first attempt hit the documented unrelated dead-but-unreaped-session timing flake), and warnings-as-errors Clippy passed. `git diff --check` passed. The process-level resident E2E is 3/3; focused resident/service/Electron tests are 65/65.

The exact packaged smoke used private roots only (`/tmp/pk-2869`, private daemon, fake Pi, private browser profile, Xvfb `:135`, no compatibility flags). It proved hostile HTTP navigation, process/require/Electron/Amber isolation, popup denial (one native browser target after popup), unsafe `file:` redirect rejection (`chrome-error://chromewebdata/`), Pi designation/share, broker status/snapshot, visible untrusted-content approval card, approved semantic click, resident window hide with daemon/browser sockets retained, and five concurrent second-instance launches converging on one window, one renderer target, and one browser target with no `Object has been destroyed` rejection. Evidence is under `packaged-smoke-final/` in the same private root. Xvfb/XTest is a substrate limitation, not physical Linux hardware/IBus evidence; XTest approval clicking was inconclusive, while the approval was independently completed by the deterministic CDP fixture path.

A second bounded current-AppImage probe (`packaged-quit-proof2/`, private root `/tmp/pkquit2-23110`, Xvfb `:137`) ran after close/reopen stress. The real packaged `window.amber.connectHost()` path used a private fake `ssh`; it created a tracked forwarding child and owned `/tmp/amber-ssh-dJD5aY`, and closing the fakehost window killed the child and removed that directory. The real application Quit accelerator was then attempted from the local window through both CDP and XTest after the close/reopen cycle; the no-window-manager Xvfb substrate did not dispatch it (bounded 10.6 s wait, app remained resident, no inhibit marker, browser/daemon sockets stayed present). Therefore force-Quit tunnel cleanup is covered directly by the spawned-process regression and source ordering, but not claimed as a packaged force-menu observation; the exact missing substrate is a driveable application menu/accelerator under a real window manager. No real host, SSH credential, production daemon, or global Pi profile was contacted.

The resident P1 remains `partial` pending required independent reviewer re-review. The production code tested by the package and gates is exactly `CODE_TESTED_COMMIT=2fe2fccfeacab9e134c51ee90dc68d47df30ad22`; subsequent report commits are docs-only metadata/evidence updates. `mergeReady` remains **false** for the deployed-reader channel, macOS, physical Linux input, Pi 0.85 packaging, and Windows gates.

## Resident review PASS WITH FIXES follow-up (2026-09-05; implementation complete, independent signoff pending)

This follow-up implements the remaining resident P1/P2 findings without changing the Amber daemon protocol, daemon/profile configuration, or global Pi installation. Production code is `4ed8b8a0910efc0d5e84550b84440038af60c4c6`; `06a4add2df178e1f19f0e28767ed368ca2299d03` is the preceding lifecycle commit and `667ba48ef0b12d85c8f63e2700c98a2697d9394d` adds only the final readiness-error test.

- `TrackedTunnelRegistry` registers the spawned child before the next await, fences new registrations at Quit/compat start, and shares one cleanup-once promise across pending opens, window close, normal Quit, force Quit, and compat exit. Deferred `openWindow`, rejection, close-vs-Quit, duplicate cleanup, and child/runtime-directory removal are covered by real-child tests.
...
SHA-256: `fc6ffed4bf9ad291afa6c89bb96d62b1bc2abe6ae709e10071a6279470ff9d82`. Bundled static binaries are `amber` `a2b48e3d4bef35c4d304089af4a59343e745c628ab65c845d04612dac38833bb` and `amber-router` `e36374e7b0f7d53cf9c4a7dc3d7fed33d3fa6b67c9b7a3a7551d02ec92f20cdf`. Package contract inspection passed; evidence is under `/tmp/amber-tab-browser-validation/resident-review-followup-20260905-final/`.

A bounded packaged private smoke used a private static daemon, fake `ssh`, private HOME/state/runtime/profile, and Xvfb. `window.amber.connectHost('fakehost')` created the tracked fake forwarding child; closing the remote target returned `Target is closing`, removed `/tmp/amber-ssh-kQvyDj`, and left no fake tunnel process. The previous packaged close/reopen/concurrent-activation evidence remains under `packaged-smoke-final/`. Explicit application-menu/accelerator Quit was **not** claimed: bare Xvfb has no driveable window-manager menu substrate, so the attempt could not dispatch and the app remained resident. Xvfb/XTest is not physical keyboard/IBus evidence. Pi 0.85 verifier remains blocked by `ERR_MODULE_NOT_FOUND: @earendil-works/pi-server`; the earlier pinned Pi 0.81 evidence remains recorded separately. Independent resident re-review, deployed-reader, macOS, physical input, Pi 0.85, and Windows gates remain open; `mergeReady` remains **false**.
