...
Full isolated gates: app `1012 passed / 1 intentional real-daemon skip`, strict typecheck, Electron build, hosted-web build, Rust all-target tests passed on the clean retry (`477` library + all integration suites; first attempt hit the documented unrelated dead-but-unreaped-session timing flake), and warnings-as-errors Clippy passed. `git diff --check` passed. The process-level resident E2E is 3/3; focused resident/service/Electron tests are 65/65.
