...
function main() {
  ...
  function renderTabBar(ws, cur) {
    ...
    var add = pill('+ pane', false, function () {
      var kind = window.prompt('kind: shell, claude, grok, codex, opencode or pi', 'shell');
      if (!kind) return;
  ...
  function tile(paneId) {
    ...
    menu.addEventListener('click', function (e) {
      ...
      var agent = s.kind === 'claude' || s.kind === 'grok' || s.kind === 'codex' || s.kind === 'opencode' || s.kind === 'pi';
      var choices = ['close'];
  ...
  var CREATE_KINDS = { shell: 1, claude: 1, grok: 1, codex: 1, opencode: 1, pi: 1 };

  function newPane(kind, cwd) {
    // Mirrors the server's CREATE_KINDS check (web.rs) so a bad kind banners
    // instead of silently no-opping.
    if (!CREATE_KINDS[kind]) { banner('kind must be shell, claude, grok, codex, opencode or pi', 'warn'); return; }
